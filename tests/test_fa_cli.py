...
def test_colab_screening_command_is_registered():
    assert "fa-run-colab-screening" in fa_cli.FA_COMMANDS
    assert "fa-run-colab-screening" in fa_cli._IMPLEMENTED


def test_colab_preflight_command_is_registered_without_project_config():
    assert "fa-colab-preflight" in fa_cli.FA_COMMANDS
    assert "fa-colab-preflight" in fa_cli._IMPLEMENTED
    ...
    assert args.root == "/content/repo"
    assert args.lock == "requirements/fa-core.lock"
    assert not hasattr(args, "config")


def test_colab_preflight_dispatch_emits_single_ready_payload(monkeypatch, capsys):
    monkeypatch.setattr(
        fa_cli,
        "_run_colab_preflight",
        lambda root, lock: {
            "status": "ready",
    ...
    args = argparse.Namespace(
        ...
        root="/content/repo",
        lock="requirements/fa-core.lock",
    )
...
def test_colab_screening_parser_requires_frozen_execution_identity(capsys):
    parser = argparse.ArgumentParser()
    subparsers = parser.add_subparsers(dest="command", required=True)
    register_fa_subcommands(subparsers)
    arguments = [
        ...
        "/content/repo",
        "--checkpoint-root",
        "/content/drive/checkpoints",
        "--scratch-root",
        "/content",
        "--git-commit",
        "a" * 40,
        "--bundle-path",
        "/content/drive/fa-study.bundle",
        "--bundle-sha256",
        "b" * 64,
        "--launch-manifest",
        "/content/drive/fa-study-launch.json",
    ]

    args = parser.parse_args(arguments)

    assert args.checkpoint_root == "/content/drive/checkpoints"
    assert args.scratch_root == "/content"
    assert args.git_commit == "a" * 40
    assert args.bundle_path == "/content/drive/fa-study.bundle"
    assert args.bundle_sha256 == "b" * 64
    assert args.launch_manifest == "/content/drive/fa-study-launch.json"
    capsys.readouterr()
...
def test_colab_screening_dispatch_emits_single_final_payload(
    monkeypatch, capsys, tmp_path
):
    monkeypatch.setattr(
        fa_cli,
        "_run_colab_screening",
        lambda config, root, args: {
            "status": "assembled",
            "count": 244,
    ...
    args = argparse.Namespace(
        command="fa-run-colab-screening",
        config=str(CONFIG_PATH),
        root=str(tmp_path),
        checkpoint_root=str(tmp_path / "checkpoints"),
        scratch_root=str(tmp_path / "scratch"),
        git_commit="a" * 40,
        bundle_path=str(tmp_path / "study.bundle"),
        bundle_sha256="b" * 64,
